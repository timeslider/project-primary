...
## TODO: [red] We can't finish this game until this algorithm is solved [/red]
	...
	while true:
		if (candidate & walls) == 0:
			if not state_index.has(candidate):
				...
				var dfs_counter = 0
				const MAX_DFS_ITERATIONS = 100_000
				# Iterative DFS
				while not dfs_stack.is_empty():
					dfs_counter += 1
					if dfs_counter > MAX_DFS_ITERATIONS:
						push_error("DFS iteration limit exceeded! Last state: %x" % dfs_stack.back().state)
						break
					...
					if current.next_child_index < current.children.size():
						...
						print("Processing child %d/%d for state %x" % [current.next_child_index+1, current.children.size(), current.state])
						...
						if state_index.has(child_state):
							...
							print("Discovered NEW state: %x (index %d)" % [child_state, next_index])
						...
						dfs_stack.pop_back()
						var current_info = state_info[current.index]
						print("Finished processing state %x (index %d). Lowlink: %d" % [current.state, current.index, current_info.lowest_reachable_index])
						# Check for SCC root
						if current_info.lowest_reachable_index == current.index:
							...
							# Pop SCC from stack
							while true:
								# Safeguard against empty stack to prevent infinite loop
								if unassigned_states.is_empty():
									push_error("Popping from empty unassigned_states stack. Current index: " + str(current.index))
									break  # Critical: Break loop when stack is empty
							
								var pop_index = unassigned_states.pop_back()
								var pop_info = state_info[pop_index]
								pop_info.is_on_stack = false
								pop_info.has_outflow = true  # Mark as processed
							...
							if not dfs_stack.is_empty():
								var parent_ctx = dfs_stack.back()
								var parent_info = state_info[parent_ctx.index]
								
								# Only update if child's lowlink is smaller
								...
								# Propagate outflow status
								parent_info.has_outflow = parent_info.has_outflow or current_info.has_outflow
								
								# Actually update the parent info
								state_info[parent_ctx.index] = parent_info

...
@onready var decrement_by_1: Button = %DecrementBy1
@onready var decrement_by_10: Button = %DecrementBy10
@onready var decrement_by_100: Button = %DecrementBy100
@onready var increment_by_1: Button = %IncrementBy1
@onready var increment_by_10: Button = %IncrementBy10
@onready var increment_by_100: Button = %IncrementBy100
...
func _on_quick_select_value_changed(value: float) -> void:
	if is_equal_approx(value, 1.0):
		...
		polyomino_index = int(value * MAX_VALUE)
...
func _on_crement_pressed(value: int) -> void:
	polyomino_index += value
...
func _update_ui(value: int) -> void:
	polyomino_index_label.text = "Polyomino index: " + str(value)
	PuzzleSelectGlobal.polyomino = Util.get_polyomino(value)
	polyomino_label.text = "Polyomino: " + String.num_uint64(PuzzleSelectGlobal.polyomino)
	quick_select.set_value_no_signal(float(value) / MAX_VALUE)
	exact_select.text = str(int(value))


func _update_polyomino() -> void:
	# Clear polyomino

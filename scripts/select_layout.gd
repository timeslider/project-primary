...
func _ready() -> void:
	...
	print("This is a heart", Util.get_polyomino_index(-1746410393489597953))
...
func update_ui(_value: int) -> void:
	PuzzleSelectGlobal.layout = Util.get_polyomino(_value)
	value_label.text = value_string + String.num_uint64(PuzzleSelectGlobal.layout)
...
func update_tiles() -> void:
	...
	for row in range(8):
		for col in range(8):
			...
			if Bitboard.get_bitboard_cell_by_col_row(PuzzleSelectGlobal.layout, col, row) == true:
	...
	#Bitboard.print_bitboard(PuzzleSelectGlobal.layout)
